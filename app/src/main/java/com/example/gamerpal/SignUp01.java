        ...
        mAuth.createUserWithEmailAndPassword(emailOrPhone, password)
                .addOnCompleteListener(this, task -> {
                    if (task.isSuccessful()) {
                        ...
                        User user = new User(emailOrPhone, displayName, username);

                        FirebaseDatabase.getInstance("https://gamepal-3912b-default-rtdb.asia-southeast1.firebasedatabase.app/")
                                .getReference("Users")
                                .child(uid)
                                .setValue(user)
                                .addOnCompleteListener(task1 -> {
                                    if (task1.isSuccessful()) {
                                        ...
                                        writeLog(displayName, emailOrPhone);
                                        ...
                                        showToast("Lỗi khi lưu thông tin người dùng: " + task1.getException().getMessage());
                                    }
                                });
                    } else {
                        showToast("Lỗi tạo tài khoản: " + task.getException().getMessage());
                    }
                });
    }

    private void writeLog(String displayName, String email) {
        DatabaseReference logsRef = FirebaseDatabase.getInstance("https://gamepal-3912b-default-rtdb.asia-southeast1.firebasedatabase.app/")
                .getReference("Logs");

        String logId = logsRef.push().getKey();
        String currentTime = new SimpleDateFormat("dd/MM/yyyy HH:mm").format(new Date());
        String logMessage = "Người dùng " + displayName + " (" + email + ") đã đăng ký thành công lúc " + currentTime;

        logsRef.child(logId).setValue(logMessage);
    }
